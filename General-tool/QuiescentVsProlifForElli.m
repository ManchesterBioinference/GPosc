...
%you want to analyse
num = xlsread('20150914_Compare_Hes1_intensity_prolif_vs_quiescent_intensity.xlsx',10);
num(isnan(num)) = 0;
...
data.exp1.data = num(:,2:6); data.exp1.bckgd = num(:,7:8);

data.exp2.data = num(:,9:13); data.exp2.bckgd = num(:,14:15);

data.exp3.data = num(:,18:23); data.exp3.bckgd = num(:,24:25);
...
num = xlsread('20150914_Compare_Hes1_intensity_prolif_vs_quiescent_intensity.xlsx',11);
num(isnan(num)) = 0;
...
% Manually enter the columns of data and bckgd for all experiments
data.exp1.data = num(:,2:14); data.exp1.bckgd = num(:,15:16);

data.exp2.data = num(:,17:28); data.exp2.bckgd = num(:,29:30);

data.exp3.data = num(:,31:37); data.exp3.bckgd = num(:,38:39);
...
meanM = zeros(size(current.data,2),1);
empvarM = zeros(size(current.data,2),1);
trendvarM = zeros(size(current.data,2),1);
...
%     [avg,m] = detrenddataNEW(raw,x,DetrendParam);
    [m,par0] = detrenddataNEW(y1,x,-4.5);
    avg = mean(raw);
    empvar = var(raw);
...
% updates total list for all pooled experiments
 ...
 meanTOT = [meanTOT;meanM];
 empvarTOT = [empvarTOT;empvarM];
 trendvarTOT = [trendvarTOT;trendvarM];
